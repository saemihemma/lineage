...
class TestOptimisticLocking:
    """Test optimistic locking for preventing state conflicts"""

    def test_version_increments_on_save(self, tmp_path):
        """Test that version increments when state is saved"""
        ...
        # Save state
        save_game_state(db, session_id, state, check_version=False)

        # Load state and check version incremented
        loaded_state = load_game_state(db, session_id)
        assert loaded_state.version == 2

        db.close()

    def test_optimistic_lock_conflict_detection(self, tmp_path):
        ...
        # Load state (version is now 2)
        loaded_state = load_game_state(db, session_id)

        # Simulate another tab/request updating the state
        another_state = load_game_state(db, session_id)
        another_state.soul_xp = 100
        save_game_state(db, session_id, another_state, check_version=False)
        # another_state is now version 3

        # Try to save old state with version check - should fail
        loaded_state.soul_xp = 200
        with pytest.raises(RuntimeError, match="State conflict detected"):
            save_game_state(db, session_id, loaded_state, check_version=True)

        db.close()

    def test_no_conflict_when_check_disabled(self, tmp_path):
        """Test that conflicts are allowed when check_version=False"""
        # Create temporary database
        db_path = tmp_path / "test.db"
        db = sqlite3.connect(str(db_path))
        db.row_factory = sqlite3.Row
        ...
        # Save old state without version check - should succeed
        save_game_state(db, session_id, loaded_state, check_version=False)
